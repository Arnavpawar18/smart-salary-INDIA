...
class EvidenceDocument(Base, TimestampMixin):
    ...
    )  # ACT, GAZETTE, CIRCULAR, NOTIFICATION, RULE, FAQ
    ...
    superseded_by_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("evidence_documents.id", ondelete="SET NULL"), nullable=True
    )

    fragments: Mapped[list["EvidenceFragment"]] = relationship(
        "EvidenceFragment", back_populates="document", cascade="all, delete-orphan"
    )
...
class EvidenceFragment(Base, TimestampMixin):
    ...
    document_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("evidence_documents.id", ondelete="CASCADE"), nullable=False
    )
    ...
    assertions: Mapped[list["EvidenceAssertion"]] = relationship(
        "EvidenceAssertion", back_populates="fragment", cascade="all, delete-orphan"
    )
...
class EvidenceAssertion(Base, TimestampMixin):
    ...
    fragment_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("evidence_fragments.id", ondelete="CASCADE"), nullable=False
    )
