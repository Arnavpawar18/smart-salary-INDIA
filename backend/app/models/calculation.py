...
class CalculationRun(Base, TimestampMixin):
    ...
    snapshot: Mapped[Optional["CalculationSnapshot"]] = relationship(
        "CalculationSnapshot", back_populates="calculation_run", uselist=False
    )
    traces: Mapped[list["CalculationTrace"]] = relationship(
        "CalculationTrace", back_populates="calculation_run", cascade="all, delete-orphan"
    )
    line_items: Mapped[list["CalculationLineItem"]] = relationship(
        "CalculationLineItem", back_populates="calculation_run", cascade="all, delete-orphan"
    )
...
class CalculationSnapshot(Base, TimestampMixin):
    ...
    tax_rule_version_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("tax_rule_versions.id"), nullable=True
    )
    pf_rule_version_id: Mapped[int | None] = mapped_column(BigInteger, ForeignKey("pf_rule_versions.id"), nullable=True)
    professional_tax_rule_version_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("professional_tax_rule_versions.id"), nullable=True
    )
...
class CalculationLineItem(Base, TimestampMixin):
    ...
    calculation_run_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("calculation_runs.id", ondelete="CASCADE"), nullable=False
    )
    category: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # GROSS, EXEMPTION, DEDUCTION, TAX_SLAB, REBATE, CESS
...
class CalculationTrace(Base, TimestampMixin):
    ...
    source_line_item_id: Mapped[int | None] = mapped_column(
        BigInteger, ForeignKey("calculation_line_items.id", ondelete="SET NULL"), nullable=True
    )
    ...
    source_line_item: Mapped[Optional["CalculationLineItem"]] = relationship(
        "CalculationLineItem", back_populates="traces"
    )
