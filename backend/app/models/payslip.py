...
class PayslipDocument(Base, TimestampMixin):
    ...
    status: Mapped[str] = mapped_column(String(50), default="UPLOADED", nullable=False)  # UPLOADED, PARSED, VALIDATED, FAILED

    employee: Mapped["Employee"] = relationship("Employee")
    extractions: Mapped[list["PayslipExtraction"]] = relationship("PayslipExtraction", back_populates="document", cascade="all, delete-orphan")
    validations: Mapped[list["PayslipValidation"]] = relationship("PayslipValidation", back_populates="document", cascade="all, delete-orphan")
...
class PayslipExtraction(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    document_id: Mapped[int] = mapped_column(BigInteger, ForeignKey("payslip_documents.id", ondelete="CASCADE"), nullable=False)
