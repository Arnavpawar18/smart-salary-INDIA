...
class TaxDeclaration(Base, TimestampMixin):
    ...
    planned_monthly_withholding: Mapped[Decimal] = mapped_column(
    ...
    items: Mapped[list["TaxDeclarationItem"]] = relationship(
        "TaxDeclarationItem", back_populates="declaration", cascade="all, delete-orphan"
    )
...
class TaxDeclarationItem(Base, TimestampMixin):
    ...
    declaration_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("tax_declarations.id", ondelete="CASCADE"), nullable=False, index=True
    )
...
class StatutoryComplianceEvent(Base, TimestampMixin):
    ...
    )  # TDS_DEPOSIT, FORM_24Q, PF_ECR, PT_RETURN, FORM_16_ISSUE
    ...
    )  # PENDING, IN_PROGRESS, FILED, DELAYED, RECONCILED
