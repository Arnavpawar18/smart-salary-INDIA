...
class PFRuleVersion(Base, TimestampMixin):
    ...
    interest_rules: Mapped[list["PFInterestRule"]] = relationship(
        "PFInterestRule", back_populates="rule_version", cascade="all, delete-orphan"
    )
...
class PFRule(Base, TimestampMixin):
    ...
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    pf_rule_version_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("pf_rule_versions.id", ondelete="CASCADE"), nullable=False
    )

    employee_epf_rate: Mapped[Decimal] = mapped_column(Numeric(10, 4), default=Decimal("0.1200"), nullable=False)  # 12%
    employer_epf_rate: Mapped[Decimal] = mapped_column(
        Numeric(10, 4), default=Decimal("0.0367"), nullable=False
    )  # 3.67%
    employer_eps_rate: Mapped[Decimal] = mapped_column(
        Numeric(10, 4), default=Decimal("0.0833"), nullable=False
    )  # 8.33%
    employer_edli_rate: Mapped[Decimal] = mapped_column(
        Numeric(10, 4), default=Decimal("0.0050"), nullable=False
    )  # 0.50%
    admin_charges_rate: Mapped[Decimal] = mapped_column(
        Numeric(10, 4), default=Decimal("0.0050"), nullable=False
    )  # 0.50%
