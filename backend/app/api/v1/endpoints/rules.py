...
def get_rules_summary(db: Session = Depends(get_db)):
    ...
    return {
        "tax_rule_versions": [
            {"version": tr.version_code, "regime": tr.regime, "status": tr.status} for tr in tax_rules
        ],
        "pf_rule_versions": [{"version": pf.version_code, "status": pf.status} for pf in pf_rules],
        "professional_tax_configured_states": [
            {"version": pt.version_code, "state_id": pt.state_id} for pt in pt_rules
        ],
    }
