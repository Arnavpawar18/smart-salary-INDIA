"""
SmartSalary India — 100,000+ Adversarial System Validation Runner
...
from app.core.compliance.source_registry import OfficialSourceRegistry, AuthorityTier
from app.core.compliance.state_jurisdiction_master import StateJurisdictionMaster, JurisdictionStatus
...
from app.engine.oracle.independent_oracle import IndependentRegulatoryOracle, OracleSalaryInput
...
class ValidationRunner:
    ...
    def run_all(self):
        ...
        print(f"\n[100k Validation] COMPLETED in {elapsed:.2f}s! Total Scenarios: {self.total_scenarios:,}, Passed: {self.passed_scenarios:,}, Failed: {self.failed_scenarios}", flush=True)
    ...
    def record_scenario(self, domain: str, scenario_id: str, input_payload: dict, output_payload: dict, is_pass: bool, is_blocked: bool = False):
    ...
    def validate_income_tax_domain(self, count: int):
        ...
        critical_thresholds = [
            Decimal("0"), Decimal("100"), Decimal("10000"), Decimal("300000"), Decimal("400000"),
            Decimal("700000"), Decimal("800000"), Decimal("1000000"), Decimal("1200000"),
            Decimal("1500000"), Decimal("1600000"), Decimal("2000000"), Decimal("2400000"),
            Decimal("5000000"), Decimal("10000000"), Decimal("20000000"), Decimal("50000000"),
        ]
        boundaries = []
        ...
        for i in range(count):
            ...
            if regime == TaxRegime.NEW:
                ...
                res_oracle = IndependentRegulatoryOracle.calculate_fy2025_26_old(gross, "KA", sec_80c=sec_80c, sec_80d=sec_80d)
            ...
            if not is_pass:
                ...
                if self.tax_mismatches <= 3:
                    print(f"TAX FAIL: fy={fy}, reg={regime.value}, gross={gross}, 80c={sec_80c}, 80d={sec_80d}, engine_taxable={tax_res['taxable_income']}, oracle_taxable={res_oracle.taxable_income}, engine_tax={tax_res['total_annual_tax_liability']}, oracle_tax={res_oracle.total_tax}", flush=True)

            self.latencies.append((time.perf_counter() - t0) * 1000)
            self.record_scenario(
                domain="INCOME_TAX",
                scenario_id=f"TAX-{fy}-{regime.value}-{i:05d}",
                input_payload={"fy": fy, "regime": regime.value, "gross": str(gross), "80c": str(sec_80c), "80d": str(sec_80d)},
                output_payload={"taxable_income": str(tax_res["taxable_income"]), "total_tax": str(tax_res["total_annual_tax_liability"]), "rebate": str(tax_res["section_87a_rebate"])},
                is_pass=is_pass,
            )
    ...
    def validate_pf_domain(self, count: int):
        ...
        for i in range(count):
            ...
            # Statutory PF Ceiling Verification (₹15,000 / month)
            ...
            self.record_scenario(
                ...
                output_payload={"employee_pf": str(res_pf.monthly_employee_epf), "employer_eps": str(res_pf.monthly_employer_eps)},
                is_pass=is_pass,
            )
    ...
    def validate_salary_components_domain(self, count: int):
        ...
        for i in range(count):
            ...
            self.record_scenario(
                ...
                output_payload={"basic": str(norm.basic_salary), "hra": str(norm.hra), "special": str(norm.special_allowance)},
                is_pass=is_pass,
            )
    ...
    def validate_tax_regime_domain(self, count: int):
        ...
        for i in range(count):
            ...
            res_old = TaxCalculator.calculate_tax(TaxCalculationInput("2026-27", TaxRegime.OLD, gross, section_80c=Decimal("150000.00"), section_80d=Decimal("25000.00")), tax_rules_old)

            is_pass = (res_new["standard_deduction"] == Decimal("75000.00")) and (res_old["standard_deduction"] == Decimal("50000.00"))
            self.latencies.append((time.perf_counter() - t0) * 1000)
            self.record_scenario(
                domain="TAX_REGIME",
                scenario_id=f"REGIME-{i:05d}",
                input_payload={"gross": str(gross)},
                output_payload={"new_tax": str(res_new["total_annual_tax_liability"]), "old_tax": str(res_old["total_annual_tax_liability"])},
                is_pass=is_pass,
            )
