...
def test_exact_49_domain_tables_registered():
    """Verify that exactly 52 domain tables are declared in Base.metadata (including M8.1 audit models and verification_tokens)."""
    declared_tables = Base.metadata.tables
    assert len(declared_tables) == 52, f"Expected exactly 52 tables, found {len(declared_tables)}"


def test_domain_groups_sum_to_49():
    """Verify that domain group mapping contains all 52 unique tables without duplication."""
    all_grouped_tables = []
    for domain, tables in DOMAIN_GROUPS.items():
        all_grouped_tables.extend(tables)

    assert len(all_grouped_tables) == 52, f"Expected 52 grouped tables, found {len(all_grouped_tables)}"
    assert len(set(all_grouped_tables)) == 52, "Duplicate table names detected across domain groups"
