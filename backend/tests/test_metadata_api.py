...
def test_metadata_api_contract():
    """Verify response structure and 49-table count of /api/v1/metadata/schema-summary (including organizations, payroll core, and compliance)."""
    response = client.get("/api/v1/metadata/schema-summary")
    assert response.status_code == 200

    data = response.json()
    assert "total_domain_tables" in data
    assert data["total_domain_tables"] == 49
    ...
    # Verify that the sum of tables in domains equals 49
    total_in_domains = sum(len(tables) for tables in data["domains"].values())
    assert total_in_domains == 49
